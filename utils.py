...
def computeAccuracy( pred, labelIndexBatch, maskBatch, numClasses = 21 ):
    """
    computes unnormalized confusion matrix of each batch of labels.
    return: array, shape = (numClasses, numClasses)
    """
    ...
    gt = gt.squeeze(1)
    mask = maskBatch.cpu().data.numpy()   # use to crop image
    mask = mask.squeeze(1)
...
def save_label(label, mask, cmap, name, nrows, ncols ):
    """
    function turns the label into an image using the given color map and saves 
    the image at the assigned location
    """
...
def compute_batch_accuracy(confcounts, numClasses):
    """Computer batch accuracy

    Args:
        confcounts (array): shape n_class x n_class. Confusion matrix
        numClasses (int): number of classes

    Returns:
        float: mean_avg, class_acc
    """
    ...
    class_accuracy = {f"label{i}": accuracy[i] for i in range(numClasses)}
    return np.mean(accuracy), class_accuracy