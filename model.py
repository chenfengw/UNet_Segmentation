...
class encoderDilation(nn.Module ):
    ...
    def __init__(self ):
        ...
        self.b3_1 = ResBlock(128, 256, 1, 2) # stride 1, dilation 2
        self.b3_2 = ResBlock(256, 256, 1, 2) # stride 1, dilation 2

        self.b4_1 = ResBlock(256, 512, 1, 4) # stride 1, dilation 4
        self.b4_2 = ResBlock(512, 512, 1, 4) # stride 1, dilation 4

    def forward(self, im):
        """
        im: input images, shape = (batch_size, n_channel, h, w)
        """
        x1 = F.relu(self.bn1(self.conv1(im)), inplace=True)
        x2 = self.b1_2(self.b1_1(self.maxpool(x1 ) ) )
        x3 = self.b2_2(self.b2_1(x2 ) )
...
class PyramidPooling(nn.Module):
    ...
    def forward(self, x):
        """
        x is output of last resnet block
        """
        size = (x.shape[-2], x.shape[-1])
        b1 = F.interpolate(self.branch1(x), size = size, mode = "bilinear", align_corners = True)
        b2 = F.interpolate(self.branch2(x), size = size, mode = "bilinear", align_corners = True)
        b3 = F.interpolate(self.branch3(x), size = size, mode = "bilinear", align_corners = True)
        b4 = F.interpolate(self.branch4(x), size = size, mode = "bilinear", align_corners = True)
        
        return torch.cat([b1, b2, b3, b4, x], dim = 1)


class encoderSPP(nn.Module):
    """
    Encoder for 2C with special pyramid pooling
    """
    def __init__(self):
        ...
        # add pyramid pooling
        self.pyramid_pooling = PyramidPooling(512)
    ...
    def forward(self, im):
        ...
        # add pyramid polling
        x5_pool = self.pyramid_pooling(x5) # chan 1024
        return x1, x2, x3, x4, x5_pool


class decoderSPP(nn.Module ):
    """
    Decoder for 2C with pyramid pooling
    """
    def __init__(self, isSpp = False ):
        super(decoderSPP, self).__init__()
        self.conv1 = nn.Conv2d(512*2+256+128, 512, 3, 1, 1, bias=False )
        self.bn1 = nn.BatchNorm2d(512 )
        self.conv1_1 = nn.Conv2d(512, 21, 3, 1, 1, bias=False )
        self.bn1_1 = nn.BatchNorm2d(21)
        self.conv2 = nn.Conv2d(64+21, 21, 3, 1, 1, bias=False )
